...
if (!isset($_SESSION["lang"])) { $_SESSION["lang"] = "en";  header("Location: cart.php");}
if (isset($_POST["lang"])) { $_SESSION["lang"] = $_POST["lang"];  header("Location: cart.php");}

require "lang/cart-".$_SESSION["lang"].".php";
...
<nav class="breadcrumb-section theme1 breadcrumb-bg1" style="
        background-image: url('data:image/jpg;charset=utf8;base64,<?php echo base64_encode($image2); ?>')">
  <div class="container">
    <div class="row">
      <div class="col-12">
...
<section class="whish-list-section theme1 pb-70">
    <div class="container grid-wraper">
        <div class="row">
            <div class="col-12" >
                <h3 class="title pb-25 text-center text-md-left text-capitalize" style="text-align: right;"><?= $_TXT[69]; ?></h3>
                <?php 
                if (!isset($_GET['add'])){
                  
                }else{
                  echo '<h3 class="title pb-25 text-center text-md-left text-capitalize">'.$_TXT[70].'</h3>';
                } 
                ?>
                <?php if(isset($_GET['true'])){
                  echo '<h3 class="title pb-25 text-center text-md-left text-capitalize">'.$_TXT[71].'</h3>';
                  $_GET['true'] = null;
                ...
                <?php 
if (!isset($_SESSION['id1'])){
  echo '  <h3 class="title pb-25 text-center text-md-left text-capitalize">'.$_TXT[72].'</h3>';
} else{
  ...
  if (isset($_POST['product'])){
   ...
   while ($row = mysqli_fetch_assoc($result)){
    ...
    $ar_product_name = $row['ar_product_name'];
    $store_name= $row['store_name'];
    $ar_store_name = $row['ar_store_name'];
    ...
    $query1="SELECT * from product_image where store_id = '$store_id' and product_name='$product_name'  ";
  ...
  $query5="SELECT * from cart where id_user = '$id1' and id_product = '$id' and color = '$new_color'";
  $result5 = mysqli_query($conn,$query5);
  if (mysqli_num_rows($result5) > 0){
    echo '<h3 class="title pb-25 text-center text-md-left text-capitalize">'.$_TXT[73].'</h3>';
  }else {
      ...
      if(preg_match_all('/\d+/', $category , $category1)){
        $category1 = end($category1[0]);
      }
      ...
      $product_category1 = preg_replace('/\d/', '', $category);


      $query="INSERT INTO `cart`(`id_user`, `id_product`,`store_id`, `product_image`, `product_name`,`ar_product_name`, `category`,`sub_category`,`color`,`store_logo`, `store_name`,`ar_store_name`, `price`, `qty`) VALUES
      ('$id1','$id','$store_id','$piece','$product_name','$ar_product_name','$category2','$product_category1','$new_color','$store_logo','$store_name','$ar_store_name','$price','$qty');";
 
       $result = mysqli_query($conn,$query);
 
     echo '<h3 class="title pb-25 text-center text-md-left text-capitalize">'.$_TXT[74].'</h3>';
    
     echo '<script> window.location.href = "cart.php?add=add"; </script>';
  ...
  if (isset($_GET['id1']) && isset($_GET['id'])){
    ...
    while ($row = mysqli_fetch_assoc($result)){
     ...
     $product_price = $row['product_price'];
     $discount = $row['discount'];
     $product_colors2 = $row['product_colors'];
   ...
   $product_colors3 = explode(",", $product_colors2);

   foreach ($product_colors3 as $product_colors4){

   }


   $query5="SELECT * from cart where id_user = '$id1' and id_product = '$id' and color = '$product_colors4' ";
   $result5 = mysqli_query($conn,$query5);
   if (mysqli_num_rows($result5) > 0){
     echo '<h3 class="title pb-25 text-center text-md-left text-capitalize">'.$_TXT[73].'</h3>';
   }else {
    ...
    $product_category1 = preg_replace('/\d/', '', $category);
       ...
       $query="INSERT INTO `cart`(`id_user`, `id_product`,`store_id`, `product_image`, `product_name`,`ar_product_name`, `category`,`sub_category`,`color`, `store_logo`, `store_name`,`ar_store_name`, `price`) VALUES
       ('$id1','$id','$store_id','$piece','$product_name','$ar_product_name','$category2','$product_category1','$product_colors4','$store_logo','$store_name','$ar_store_name','$price');";
                ...
                <div class="table-responsive">
                    <table class="table mb-0">
                        <thead class="thead-light">
                            <tr>
                                <th class="text-center" scope="col"><?= $_TXT[75]; ?></th>
                                <th class="text-center" scope="col"><?= $_TXT[76]; ?></th>
                                <th class="text-center" scope="col"><?= $_TXT[77]; ?></th>
                                <th class="text-center" scope="col"><?= $_TXT[78]; ?></th>
                                <th class="text-center" scope="col"><?= $_TXT[79]; ?></th>
                                <th class="text-center" scope="col"><?= $_TXT[80]; ?></th>
                                <th class="text-center" scope="col"><?= $_TXT[81]; ?></th>
                                <th class="text-center" scope="col"><?= $_TXT[82]; ?></th>
                                <th class="text-center" scope="col"><?= $_TXT[83]; ?></th>
                            </tr>
                        </thead>
                        <tbody>
                        <?php
                          $id1 = $_SESSION['id1'];
                            ...
                            while ($row1 = mysqli_fetch_assoc($result2)){
                              $id2 = $row1['id']; 
                              $product_image= $row1['product_image'];
                              $product_name = $row1[$_TXT[84]];
                              $price = $row1['price'];
                              $id_product1 =$row1['id_product'];
                              $store_id1 = $row1['store_id'];
                              $store_logo = $row1['store_logo'];
                              $store_name = $row1[$_TXT[85]];
                              $qty = $row1['qty'];

                          ?>
                            <tr>
                                <th class="text-center" scope="row">
                                <a href="single-product-configurable.php?id_store=<?php echo $store_id1?>&id_product=<?php echo $id_product1;  ?>&storename=<?php echo $store_name; ?>" class="image">
                                    <img src="Admin Store PDD/material-dashboard-master/upload/<?php echo $product_image; ?>" alt="img">
                                </a>
                                </th>
                                <td class="text-center">
                                <a href="single-product-configurable.php?id_store=<?php echo $store_id1?>&id_product=<?php echo $id_product1;  ?>&storename=<?php echo $store_name; ?>" class="image">
                                    <span class="whish-title"><?php echo $product_name; ?></span>
                                </a>
                                </td>
                                <th class="text-center" scope="row">
                                <a href="shop-grid-4-column-store.php?id_store=<?php echo $store_id; ?>&store-name=<?php echo $store_name; ?>&rele=rele">
                                  <img src="data:image/jpg;charset=utf8;base64,<?php echo $store_logo; ?>" alt="img">
                                </a>
                              </th>
                              <td class="text-center">
                              <a href="shop-grid-4-column-store.php?id_store=<?php echo $store_id; ?>&store-name=<?php echo $store_name; ?>&rele=rele">
                                  <span class="whish-title"><?php echo $store_name;  ?></span>
                            </a>
                              </td>
                                <td class="text-center">
                                    <span class="badge badge-danger position-static my-badge"><?= $_TXT[86]; ?></span>
                                </td>
                                <td class="text-center">
                                    <span class="whish-list-price">
                                    <?php echo $price ?> <?= $_TXT[87]; ?>
                                    </span></td>
                                ...
                                <td class="text-center">
                                    <a href="checkout.php?id_product=<?php echo $id_product1; ?>&qty=<?php echo $qty; ?>" class="btn theme-btn--dark1 btn--xl text-uppercase"><?= $_TXT[88]; ?></a>
                                </td>
                            </tr>
                            <?php }
                            }else {
                              echo '<h3 class="title pb-25 text-center text-md-left text-capitalize">'.$_TXT[89].'</h3>';
                            }
                             ?>
                        </tbody>
                    </table>
