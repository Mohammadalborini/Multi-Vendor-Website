...
if (!isset($_SESSION["lang"])) { $_SESSION["lang"] = "en"; header("Location: Wish List.php");}
if (isset($_POST["lang"])) { $_SESSION["lang"] = $_POST["lang"];  header("Location: Wish List.php");}

require "lang/Wish List-".$_SESSION["lang"].".php";
...
<nav class="breadcrumb-section theme1 breadcrumb-bg1" style="
        background-image: url('data:image/jpg;charset=utf8;base64,<?php echo base64_encode($image7); ?>')">
  <div class="container">
    <div class="row">
      <div class="col-12">
...
<section class="compare-section theme1 pb-70">
    <div class="container grid-wraper">
        <div class="row">
            <div class="col-12">
                <h3 class="title pb-25 text-center text-md-left text-capitalize"><?= $_TXT[66]; ?></h3>

              <?php if(isset($_GET['true'])){
                  echo '<h3 class="title pb-25 text-center text-md-left text-capitalize">'.$_TXT[68].'</h3>';
                  $_GET['true'] = null;
                ...
                <?php 
if (!isset($_SESSION['id1'])){
  echo '  <h3 class="title pb-25 text-center text-md-left text-capitalize">'.$_TXT[69].'</h3>';
} else{
  ...
  if (isset($_GET['id1']) && isset($_GET['id'])){
   ...
   while ($row = mysqli_fetch_assoc($result)){
    ...
    $ar_short_description = $row['ar_short_description'];
  ...
  $query5="SELECT * from wishlist where id_user = '$id1' and product_id = '$id'  ";
  $result5 = mysqli_query($conn,$query5);
  if (mysqli_num_rows($result5) > 0){
    ...
    $query="INSERT INTO `wishlist`( `id_user`, `product_id`, `product_image`, `product_name`,`ar_product_name`, `price`, `description`,`ar_short_description`, `colors`) VALUES
     ('$id1','$id','$piece','$product_name','$ar_product_name','$product_price','$description2','$ar_short_description','$color') ";
      $result = mysqli_query($conn,$query);
                ...
                <div class="table-responsive">
                    <table class="table">
                        <thead class="thead-light">
                            <th scope="col"><?= $_TXT[72]; ?></th>
                            <th scope="col"><?= $_TXT[73]; ?></th>
                            <th scope="col"><?= $_TXT[74]; ?></th>
                            <th scope="col"><?= $_TXT[75]; ?></th>
                            <th scope="col"><?= $_TXT[76]; ?></th>
                            <th scope="col"><?= $_TXT[77]; ?></th>
                        </thead>
                        <tbody>
                          <?php
                          $id1 = $_SESSION['id1'];
                            ...
                            while ($row1 = mysqli_fetch_assoc($result2)){
                              $id2 = $row1['id']; 
                              $product_image= $row1['product_image'];
                              $product_name = $row1[$_TXT[78]];
                              $price = $row1['price'];
                              $description	 = $row1[$_TXT[79]];
                              $colors = $row1['colors'];
                          ?>
                        <tr  class="text-center">
                                <td>
                                 <img src="Admin Store PDD/material-dashboard-master/upload/<?php echo $product_image; ?>" alt="img">
                                    <span class="sub-title d-block"><?php echo $product_name; ?></span>
                                    <a href="cart.php?id1=<?php echo $id1; ?>&id=<?php echo $id;?>" class="btn  theme-btn--dark1 btn--xl"> <?= $_TXT[80]; ?></a>
                                </td>
                                <td class="text-center">
                                    <span class="whish-list-price">
                                        <?php echo $price; ?> <?= $_TXT[81]; ?>
                                    </span>
                                </td>
                                ...
                                <td class="text-center">
                                    <span class="badge badge-danger position-static my-badge"><?= $_TXT[82]; ?></span>
                                </td>
                                <td class="text-center">
                                  <?php $piece1 = explode(",", $colors); 
                                    for($i=0; $i<count($piece1); $i++ ){
                                      echo ' <input  type="color" value="'.$piece1[$i].'" disabled>';
                                    }
                                ...
                                <td class="text-center">
                                  <form action="" method="GET">
                                    <input type="hidden" name="name1" value="<?php echo $id1 ?>"/>
                                    <input type="hidden" name="name2" value="<?php echo $id2; ?>"/>
                                    <button name="submit1" type="submit" style="color:#fff; padding:15px; background:red;"> 
                                    <?= $_TXT[83]; ?>
                                  </button>
                                </form>
                            ...
                            <?php }
                              ...
                              echo '<h3 class="title pb-25 text-center text-md-left text-capitalize">'.$_TXT[84].'</h3>';
                            }
                             ?>
...
$query4="SELECT * from wishlist where id = '$id2' ";
$result4 = mysqli_query($conn,$query4);
$sdf = mysqli_num_rows($result4);
if (mysqli_num_rows($result4) > 0){
  ...
  $query3="DELETE FROM `wishlist` where id_user = '$id1' and id = '$id2' ";
  $result3 = mysqli_query($conn,$query3);

  echo '<script> window.location.href = "Wish List.php?true=true"; </script>';
}
}
