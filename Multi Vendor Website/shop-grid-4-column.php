...
if (!isset($_SESSION["lang"])) { $_SESSION["lang"] = "en";  header("Location: shop-grid-4-column.php?product=$product");}
if (isset($_POST["lang"])) { $_SESSION["lang"] = $_POST["lang"];  header("Location: shop-grid-4-column.php?product=$product");}

require "lang/shop-grid-4-column-".$_SESSION["lang"].".php";
...
<?php if(isset($_GET['product'])){
    ...
    $query="SELECT * from orders where product_category = '$product' ";
    $result = mysqli_query($conn,$query);
    if (mysqli_num_rows($result) > 0){
    while ($row = mysqli_fetch_assoc($result)){
        $product1 = $row[$_TXT[68]];
    }
    $product1 = preg_replace('/\d/', '', $product1);
    ...
    if($product == 'Clothing'){
        ...
        if($_SESSION['lang'] == 'en'){
            $product1 = 'Accessories';
        }else{
            $product1 = 'مُكَمِّلات';
        }
    }elseif($product == 'Perfumes'){
        if($_SESSION['lang'] == 'en'){
            $product1 = 'Perfumes';
        }else{
            $product1 = 'عطور';
    ...
    elseif($product == 'PHONE ACCESSORIES8'){
        if($_SESSION['lang'] == 'en'){
            $product1 = 'Phone accessories';
        }else{
            $product1 = 'ملحقات الهاتف';
        }
    }
    elseif($product == 'hand made9'){
        if($_SESSION['lang'] == 'en'){
            $product1 = 'Hand made';
        }else{
            $product1 = 'صنع يدوي';
        }
    }elseif($product == 'arts10'){
        if($_SESSION['lang'] == 'en'){
            $product1 = 'Arts';
        }else{
            $product1 = 'الفنون';
        ...
        $product1 = preg_replace('/\d/', '', $product);
                ...
                <div class="col-12 col-md-6 mb-3 mb-md-0">
                    <nav class="shop-grid-nav">
                        <ul class="nav nav-pills align-items-center" id="pills-tab" role="tablist">
                            ...
                            <?php
                            if ($num == ''){
                                $query3="SELECT * from orders where product_category = '$product' ";
                                $result3 = mysqli_query($conn,$query3);
        ...
        <div class="tab-content" id="pills-tabContent">
            ...
            <div class="tab-pane fade show active" id="pills-home" role="tabpanel" aria-labelledby="pills-home-tab">
                ...
                if ($num == '') {
                    ...
                    $store_name = $row[$_TXT[71]];
                    $store_logo = $row['store_logo'];
                    $product_name = $row['product_name'];
                    $product_name1 = $row[$_TXT[72]];
                    ...
                    <div class="col-sm-6 col-md-4 col-lg-3 mb-30">
                        <div class="card product-card">
                            <div class="card-body p-0">
                                ...
                                <h3 class="title"><a href="shop-grid-4-column-store.php?id_store=<?php echo $id_store?>&store-name=<?php echo $store_name; ?>&rele=rele"> <?php echo $store_name; ?></a></h3> 
                                    ...
                                    <?php
                             if (!($discount) == null) {
                               echo '<h6 class="product-price"><del class="del">'.$product_price.' '.$_TXT[73].'</del>
                               <span class="onsale">'.$discount.' '.$_TXT[73].'</span></h6>';
                            }else {
                                echo '<h6 class="product-price"> <span class="onsale">'.$product_price.' '.$_TXT[73].'</span></h6>';
                            }
                            ?>
...
$new_num= 0;
    ...
    while ($row4 = mysqli_fetch_assoc($result4)){
      $product_category3 = $row4['product_category'];

  if(preg_match_all('/\d+/', $product_category3, $numbers2)){
    $lastnum2 = end($numbers2[0]);

    if ($num ==  $lastnum2)
     {

    $id = $row4['id'];
    $id_store = $row4['store_id'];
    $store_name = $row4[$_TXT[71]];
    $store_logo = $row4['store_logo'];
    $product_name = $row4['product_name'];
    $product_name1 = $row4[$_TXT[72]];
    $uploaded_on = $row4['uploaded_on'];
...
if ($new_num === 0 ){
    echo '<h6 class="product-price"> <span class="onsale">'.$_TXT[74].' </span></h6>';
 }
    ...
    if ($num == '') {
                    ...
                    $product_name = $row['product_name'];
                    $product_name1 = $row[$_TXT[72]];
                    $short_description = $row[$_TXT[75]];
                    $uploaded_on = $row['uploaded_on'];
                    $product_price = $row['product_price'];
                    $discount = $row['discount'];

                    $full_description = $row[$_TXT[76]];
                    $product_material = $row[$_TXT[77]];
          ...
          if ($num ==  $lastnum2)
           {
                    ...
                    $product_name = $row4['product_name'];
                    $product_name1 = $row4[$_TXT[72]];
                    $short_description = $row4[$_TXT[75]];
                    $uploaded_on = $row4['uploaded_on'];
                    $product_price = $row4['product_price'];
                    $discount = $row4['discount'];

                    $full_description = $row4[$_TXT[76]];
                    $product_material = $row4[$_TXT[77]];
                    $products_sizes = $row4['products_sizes'];
