...
if (isset($_GET['id'])){
  $id = $_GET['id'];
  $_SESSION['id'] = $id;
  $result = $conn->query("SELECT * FROM sign_up WHERE id='$id'") or die($conn->error);
    ...
    $_SESSION['first_card'] =  $row['first_card'];
    $_SESSION['second_card'] =  $row['second_card'];
    $_SESSION['third_card'] =  $row['third_card'];
    $_SESSION['fourth_card'] =  $row['fourth_card'];
    $_SESSION['location'] =  $row['location'];
    $store_logo = base64_encode($row['store_logo']);
  }}

  if (!isset($_SESSION["lang"])) { $_SESSION["lang"] = "en"; }
if (isset($_POST["lang"])) { $_SESSION["lang"] = $_POST["lang"]; }

require "lang/info_edit_".$_SESSION["lang"].".php";
...
<div class="modal">
            <form method="POST">
          <button type="submit" class="btn" name="lang" value="en"> <?=$_TXT[22]?> </button>
          <button type="submit" class="btn" name="lang" value="ar"> <?=$_TXT[21]?> </button>
        </form>
  <form class="modal-content" action="edit_profile_process.php" method="POST" enctype="multipart/form-data"> 
    <div class="container">

    <div style="text-align:<?=$_TXT[24]?>;">
      <h1 style="text-align: center;"><?=$_TXT[0]?></h1>
      <hr>
      <label for="store"><b><?=$_TXT[1]?></b></label>
      <input type="text"  name="store" value="<?php echo $_SESSION['store_name']; ?>" required>

      
      <label for="image"><b><?=$_TXT[2]?></b></label><p style="color: red;">* <?=$_TXT[3]?></p>
      <img src="data:image/jpg;charset=utf8;base64,<?php echo($store_logo); ?>"  style="  display: block;
                    width: 25%;
                    height: auto; float:<?=$_TXT[24]?>; "alt="" ><br>
      <input type="file" name="image" required >

      <label for="email"><b><?=$_TXT[4]?></b></label>
      <input type="email"  name="email" value="<?php echo $_SESSION['email']; ?>" required>

      <label for="number"><b><?=$_TXT[5]?></b></label>
      <input type="text" name="number" value="<?php echo $_SESSION['phone']; ?>" required>

      <label for="user"><b><?=$_TXT[6]?></b></label>
      <input type="text"  name="user" value="<?php echo $_SESSION['username']; ?>" required>

      <label for="psw"><b><?=$_TXT[7]?></b></label>
      <input type="text" name="psw" id="pass" value="<?php echo $_SESSION['password']; ?>" onkeyup="active()" required>

      <label for="psw-repeat"><b><?=$_TXT[8]?></b></label>
      <input type="text"  name="psw-repeat" id="confirm_pass" onkeyup="active_2()" value="<?php echo $_SESSION['re_password']; ?>" required>
      <span id="wrong_pass_alert" style="text-align: center;"></span><br>

      <?php
       if($_SESSION['payment'] == 'wallet'){
        echo '<label>
        <input type="checkbox"  style="margin-bottom:15px" value="wallet" name="chek[]" checked>'.$_TXT[9].'
        </label><br>
        <label>
        <input type="checkbox"  style="margin-bottom:15px" value="bank IBAN" name="chek[]">'.$_TXT[10].'
      </label><br>';
      }elseif($_SESSION['payment'] == 'bank IBAN'){
        echo '  <label>
        <input type="checkbox"  style="margin-bottom:15px" value="bank IBAN" name="chek[]" checked>'.$_TXT[10].'
      </label><br>
      <label>
        <input type="checkbox" style="margin-bottom:15px" value="wallet" name="chek[]" >'.$_TXT[9].'
        </label><br>';
      }else{
        echo'<label>
        <input type="checkbox"  style="margin-bottom:15px" value="wallet" name="chek[]" checked> '.$_TXT[9].'
        ...
        <label>
        <input type="checkbox"  style="margin-bottom:15px" value="bank IBAN" name="chek[]" checked>bank '.$_TXT[10].'
      </label><br>';
      } ?>
      <label for="card" style="color: #000; font-size: 20px;"><b><?=$_TXT[11]?></b></label>
      <input type="text" placeholder="<?=$_TXT[12]?>" name="card1" value="<?php echo $_SESSION['first_card']; ?>" required>
      <input type="text" placeholder="<?=$_TXT[13]?>" name="card2" value="<?php echo $_SESSION['second_card']; ?>" required>
      <input type="text" placeholder="<?=$_TXT[14]?>" name="card3" value="<?php echo $_SESSION['third_card']; ?>" required>
      <input type="text" placeholder="<?=$_TXT[15]?>" name="card4" value="<?php echo $_SESSION['fourth_card']; ?>" required>
   
      <label for="add"><b><?=$_TXT[16]?></b></label><p style="color: red;">* <?=$_TXT[16]?><br>
        1- <?=$_TXT[17]?><br>
        2- <?=$_TXT[18]?><br>
        3- <?=$_TXT[19]?></p>
        </div>
      <input type="text" value="<?php echo $_SESSION['location']; ?>" name="add" required>
      <div style="display: inline-block;">
      <label  style="color: #000; font-size: 20px;">Shop Category (please enter the shop category)</label>
      <ul>
        <li><input type="checkbox" name="shop[]" id="option1" value="shose"><label for="option1"> SHOES</label>
            <ul>
              ...
              <li><label><input type="checkbox" class="subOption1" value="female2"  name="shop[]"> Female</label></li>
              <li><label><input type="checkbox" class="subOption1" value="kids3"  name="shop[]"> Kids</label></li>
            </ul>
        </li>
        <li><input type="checkbox" name="shop[]" id="option2" value="clothing"><label for="option2"> CLOTHING</label>
            <ul>
              <li><label><input type="checkbox" class="subOption2" value="male4"  name="shop[]"> Male</label></li>
              <li><label><input type="checkbox" class="subOption2" value="female5"  name="shop[]"> Female</label></li>
              <li><label><input type="checkbox" class="subOption2" value="kids6"  name="shop[]"> Kids</label></li>
            </ul>
        </li>
        <li><input type="checkbox" name="shop[]" id="option3" value="bags"><label for="option3"> BAGS</label>
          <ul>
            <li><label><input type="checkbox" class="subOption3" value="male7"  name="shop[]"> Male</label></li>
            <li><label><input type="checkbox" class="subOption3" value="female8"  name="shop[]"> Female</label></li>
            <li><label><input type="checkbox" class="subOption3" value="kids9"  name="shop[]"> Kids</label></li>
          </ul>
      </li>

      <li><input type="checkbox" name="shop[]" id="option4" value="accessories"><label for="option4">ACCESSORIES</label>
        <ul>
          <li><label><input type="checkbox" class="subOption4" value="hand_made"  name="shop[]"> Hand Made</label></li>
        </ul>
    </li>

    <li><input type="checkbox" name="shop[]" id="option5" value="perfumes"><label for="option5"> PERFUMES</label>
      <ul>
        <li><label><input type="checkbox" class="subOption5" value="male10"  name="shop[]"> Male</label></li>
        <li><label><input type="checkbox" class="subOption5" value="female11"  name="shop[]"> Female</label></li>
        <li><label><input type="checkbox" class="subOption5" value="kids12"  name="shop[]"> Kids</label></li>
      </ul>
   </li>
        <li>
          <input type="checkbox" id="option6"  name="shop[]" value="gaming"><label for="option6"> GAMING</label>
          <ul>
            <li><label><input type="checkbox" class="subOption6" value="gaming_accessories"  name="shop[]"> Gaming Accessories</label></li>
            <li><label><input type="checkbox" class="subOption6" value="figurines"  name="shop[]"> Figurines</label></li>
          </ul>
        </li>
        <li><input type="checkbox" name="shop[]" id="option7" value="makeup"><label for="option7"> MAKEUP</label></li>
        <li><input type="checkbox" name="shop[]" id="option8" value="PHONE_ACCESSORIES"><label for="option8"> PHONE ACCESSORIES</label></li>
        <li><input type="checkbox" name="shop[]" id="option9" value="hand_made"><label for="option9"> HAND MADE</label></li>
        <li><input type="checkbox" name="shop[]" id="option10" value="arts"><label for="option10"> Arts</label></li>
      </ul> 
      </div>
      ...
      <li><input type="checkbox" name="rshop[]" id="roption4" value="ملحقات"><label for="roption4">ملحقات</label>
        <ul>
          <li><label><input type="checkbox" class="rsubOption4" value="صنع يدوي"  name="rshop[]"> صنع يدوي</label></li>
        </ul>
    </li>
    ...
    <li><input type="checkbox" name="rshop[]" id="roption5" value="العطور"><label for="roption5"> العطور</label>
      <ul>
        ...
        <li>
          ...
          <ul>
            <li><label><input type="checkbox" class="rsubOption6" value="ملحفات العاب"  name="rshop[]"> ملحقات الألعاب</label></li>
            <li><label><input type="checkbox" class="rsubOption6" value="التماثيل"  name="rshop[]"> التماثيل</label></li>
          </ul>
        </li>
        <li><input type="checkbox" name="rshop[]" id="option7" value="مكياج"><label for="option7"> مكياج</label></li>
        <li><input type="checkbox" name="rshop[]" id="option8" value="ملحقات الهواتف"><label for="option8"> ملحقات هواتف</label></li>
        <li><input type="checkbox" name="rshop[]" id="option9" value="منتجات يدوية"><label for="option9"> صنع يدوي</label></li>
        <li><input type="checkbox" name="rshop[]" id="option10" value="فنون"><label for="option10"> فنون</label></li>
      </ul> 
</div> 

      <div class="clearfix">
        <button type="submit" name="submit" id="create" onclick="wrong_pass_alert()" ><?=$_TXT[0]?></button>  
        <input type="hidden" name="id" value="<?php echo $id ?>">

        <a href="store info.php?username=<?php echo $_SESSION['phone'] ?>"><?=$_TXT[25]?></a>
      </div>
    </div>
  </form>
</div>
