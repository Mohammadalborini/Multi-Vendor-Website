...
require "lang/view-product-".$_SESSION["lang"].".php";
        ...
        <div class="container-fluid">
          ...
          <div class="col-md-12">
            ...
            <div class="card-header card-header-primary">
              <h4 class="card-title" style="text-align:<?=$_TXT[13]?>;"><?=$_TXT[21]?></h4>
            </div>
            <div class="card-body">
            <?php
                     $id1 = $_GET['id_product'];
                     $result = $conn->query("SELECT * FROM orders where id='$id1'") or die($conn->error);
                   ?>
            <table class="table" style="display: inline-block; overflow-x: scroll; ">
                <thead class=" text-primary" style="width:50px;">
                ...
                <tbody>
                <?php while ($row = $result->fetch_assoc()): $_SESSION['product_name'] = $row['product_name'];?> 
                <tr>
                  <td>
                  ...
                  <td>
                  <?php echo $row['products_sizes']; ?>
                  </td>
                  <td>
                  <?php 
                   $pieces = $row['product_colors'];
                    $piece = explode(",", $pieces);
                    for ($i=0; $i<count($piece); $i++){
                      echo '
                      <input type="color" value="'.$piece[$i].'" name="'.$piece[$i].'"  disabled>
                      ';
                    }
                    ?>
                  </td>
                  ...
                  <td>
                  <?php echo $row['ar_product_name']; ?>
                  </td>
                  <td>
                  <?php echo $row['ar_short_description']; ?>
                  </td>
                  <td>
                  <?php echo $row['ar_full_description']; ?>
                  </td>
                  <td>
                  <?php echo $row['ar_product_material']; ?>
                  </td>
                  <td >
                  <a href="edit_product.php?username=<?php echo $username; ?>&id_product=<?php echo $row['id']; ?>" class="btn btn-primary" ><?=$_TXT[37]?></a>

                  <a href="All Prodect.php?username=<?php echo $username; ?>&id=<?php echo $row['id']; ?>&store_id=<?php echo $row['store_id']; ?>" onclick="javascript: return confirm('Are you sure?');" class="btn btn-danger" ><?=$_TXT[38]?></a>
                  </td>
                </tr>
                <?php endwhile; ?>
                </tbody>
            ...
            <div class="card-body">
                <?php
                     ...
                     $result1 = $conn->query("SELECT * FROM product_image where store_id='$id' and product_name= '$product_name' ") or die($conn->error);
                ...
                <tr>
                  ...
                  <?php 
                   $pieces1 = $row1['file_name'];
                    ...
                    for ($i=0; $i<count($piece); $i++){
                      echo '
                     <img src="../upload/'.$piece[$i].'" style="width:150px; height:auto;" />
                      ';
                    }
                    ?>
                  </td>
...
if (isset($_GET['id'])){
  ...
  $result1 = $conn->query("SELECT * FROM product_image WHERE store_id='$store_id' ") or die($conn->error);
    while ($row = $result->fetch_assoc()){
      ...
      while ($row1 = $result1->fetch_assoc()){
            ...
            $sql1 = "DELETE FROM product_image WHERE store_id='$store_id' ";
